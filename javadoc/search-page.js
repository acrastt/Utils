...
$(function () {
    ...
    expand.click(function (e) {
        ...
        if (this.parentElement.hasAttribute("open")) {
            ...
            searchInfo.attr("style", "border-width: 1px;").height(
                searchInfo.prop("scrollHeight"));
        }
    });
});
$(window).on("load", function () {
            ...
            if (r[key].length) {
                ...
                if (result.length > 20 && categoryCount > 1) {
                    var button = $("<button id='result-tab-" + key
                        + "' class='page-search-header'><span>" + categories[key]
                        + "</span>"
    ...
    function renderItem(item, table, rowColor) {
        ...
        if (item.category !== "modules") {
            $("<div/>").html(container).addClass("col-plain").addClass(
                rowColor).appendTo(table);
        }
        $("<div/>").html(link).addClass("col-last").addClass(rowColor).appendTo(
            table);
    }
